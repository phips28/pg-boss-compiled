  ...
  _createClass(Manager, [{
    ...
    value: function watch(name, options, callback) {
      ...
      if ('newJobCheckInterval' in options || 'newJobCheckIntervalSeconds' in options) options = Attorney.applyNewJobCheckInterval(
        options); else options.newJobCheckInterval = this.config.newJobCheckInterval;
    ...
    value: function setStateForJobs(ids, actionName, command) {
      return this.db.executeSql(command, [ids]).then(function (result) {
        assert(result.rowCount === ids.length,
          actionName + '(): ' + ids.length + ' jobs submitted, ' + result.rowCount + ' updated');
      });
    }
  }, {
    key: 'setState',
    value: function setState(config) {
      ...
      return Attorney.assertAsync(id, actionName + '() requires id argument').then(function () {
        ...
        return ids.length === 1 ? _this13.setStateForJob(ids[0], data, actionName, command, stateSuffix,
          bypassNotify) : _this13.setStateForJobs(ids, actionName, batchCommand);
      });
    }
  }, {
    key: 'complete',
