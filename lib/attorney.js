...
function checkConfig(config) {
  assert(config && ((typeof config === 'undefined' ? 'undefined' : _typeof(
        config)) == 'object' || typeof config == 'string'),
    'configuration assert: string or config object is required to connect to postgres');

  if (typeof config == 'string') config = { connectionString: config }; else {
  ...
  assert(!('uuid' in config) || config.uuid == 'v1' || config.uuid == 'v4',
